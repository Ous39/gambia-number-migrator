...
dashboardRouter.get('/admin/dashboard', requireAdmin, async (_req, res, next) => {
  try {
    const [rules, activeRules, operators, paymentsToday, latestVersion, transition, logs, campaign] = await Promise.all([
      ...
      query("SELECT config_value FROM app_config WHERE config_key='free_access_mode' LIMIT 1")
    ]);
    res.json({ data: { totalRules: rules.rows[0].count, activeRules: activeRules.rows[0].count, operators: operators.rows[0].count, paymentsToday: paymentsToday.rows[0].count, appVersion: '2.10.0', campaignMode: campaign.rows[0]?.config_value || 'off', lastRulesPublishDate: latestVersion.rows[0]?.published_at || null, currentTransitionSettings: transition.rows[0] ? mapTransition(transition.rows[0]) : null, recentActivity: logs.rows } });
  } catch (e) { next(e); }
});
