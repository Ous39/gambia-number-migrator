...
dashboardRouter.get('/admin/dashboard', requireAdmin, async (_req, res, next) => {
  try {
    ...
    res.json({ data: { totalRules: rules.rows[0].count, activeRules: activeRules.rows[0].count, operators: operators.rows[0].count, paymentsToday: paymentsToday.rows[0].count, appVersion: '1.0.0', lastRulesPublishDate: latestVersion.rows[0]?.published_at || null, currentTransitionSettings: transition.rows[0] ? mapTransition(transition.rows[0]) : null, recentActivity: logs.rows } });
  } catch (e) { next(e); }
});
