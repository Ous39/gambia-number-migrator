...
notificationsRouter.get('/notifications', async (req, res, next) => {
  try {
    ...
    if (!device.rowCount) return res.status(404).json({ message: 'Device not found' });
    if (device.rows[0].status === 'blocked') return res.status(403).json({ message: 'Device is blocked' });
    const rows = await query(`SELECT id,title,message,target,audience,data_json,created_at,sent_at FROM notifications
      WHERE enabled=TRUE AND status IN ('sent','partial') AND target IN ('all',$1)
        AND (audience='all' OR (audience='trial' AND $2='trial') OR (audience='subscribed' AND $2='active'))
      ORDER BY sent_at DESC LIMIT 100`, [device.rows[0].platform, device.rows[0].status]);
...
notificationsRouter.post('/admin/notifications', requireAdmin, async (req, res, next) => {
  try {
    ...
    const tokens = await query(`SELECT t.id,t.expo_push_token FROM device_push_tokens t
      JOIN devices d ON d.id=t.device_id
      WHERE t.active=TRUE AND d.status<>'blocked' AND ($1='all' OR t.platform=$1)
        AND ($2='all' OR ($2='trial' AND d.status='trial') OR ($2='subscribed' AND d.status='active'))`, [b.target,b.audience]);
    ...
    for (let i = 0; i < tokens.rows.length; i += 100) {
      ...
      try {
        const response = await fetch('https://exp.host/--/api/v2/push/send', { method: 'POST', headers: expoHeaders(), body: JSON.stringify(batch) });
        ...
        for (let index = 0; index < tickets.length; index += 1) {
          ...
          if (ticket?.status === 'ok' && ticket?.id && tokenBatch[index]?.id) {
            await query(`INSERT INTO notification_push_tickets (ticket_id,notification_id,push_token_id) VALUES ($1,$2,$3) ON CONFLICT (ticket_id) DO NOTHING`, [ticket.id,notification.id,tokenBatch[index].id]);
          }
        }
...
notificationsRouter.patch('/admin/notifications/:id/enabled', requireAdmin, async (req, res, next) => {
  try {
    ...
    const b = enabledSchema.parse(req.body);
    ...
    const updated = await query('UPDATE notifications SET enabled=$2 WHERE id=$1 RETURNING *', [notificationId,b.enabled]);
    await audit(req, b.enabled ? 'notification_enabled' : 'notification_disabled', 'notification', notificationId, before.rows[0], updated.rows[0]);
    res.json({ data: updated.rows[0] });
  } catch (e) { next(e); }
});

notificationsRouter.post('/admin/notifications/:id/check-receipts', requireAdmin, async (req, res, next) => {
  try {
    const notificationId = String(req.params.id);
    const notification = await query('SELECT id FROM notifications WHERE id=$1', [notificationId]);
    if (!notification.rowCount) return res.status(404).json({ message: 'Notification not found' });
    const pending = await query(`SELECT ticket_id,push_token_id FROM notification_push_tickets WHERE notification_id=$1 AND receipt_status='pending' ORDER BY created_at`, [notificationId]);
    for (let i = 0; i < pending.rows.length; i += 1000) {
      const batch = pending.rows.slice(i, i + 1000);
      const response = await fetch('https://exp.host/--/api/v2/push/getReceipts', { method: 'POST', headers: expoHeaders(), body: JSON.stringify({ ids: batch.map((row) => row.ticket_id) }) });
      ...
      for (const row of batch) {
        ...
        const receiptError = receiptStatus === 'error' ? String(receipt?.details?.error || receipt?.message || 'Unknown receipt error') : null;
        await query(`UPDATE notification_push_tickets SET receipt_status=$2,receipt_error=$3,checked_at=NOW() WHERE ticket_id=$1`, [row.ticket_id,receiptStatus,receiptError]);
        if (receiptError === 'DeviceNotRegistered') await query('UPDATE device_push_tokens SET active=FALSE,last_seen_at=NOW() WHERE id=$1', [row.push_token_id]);
      }
    }
    const counts = await query(`SELECT COUNT(*) FILTER (WHERE receipt_status='ok')::int AS ok_count,COUNT(*) FILTER (WHERE receipt_status='error')::int AS failed_count,COUNT(*) FILTER (WHERE receipt_status='pending')::int AS pending_count FROM notification_push_tickets WHERE notification_id=$1`, [notificationId]);
    const updated = await query(`UPDATE notifications SET receipt_ok_count=$2,receipt_failed_count=$3,receipt_checked_at=NOW() WHERE id=$1 RETURNING *`, [notificationId,counts.rows[0].ok_count,counts.rows[0].failed_count]);
    await audit(req, 'notification_receipts_checked', 'notification', notificationId, null, counts.rows[0]);
    res.json({ data: { ...updated.rows[0], pending_receipt_count: counts.rows[0].pending_count } });
  } catch (e) { next(e); }
});
