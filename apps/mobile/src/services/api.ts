...
export async function verifyPaymentOtp(reference: string, otp: string, deviceId: string) { const data = await request<{ data: any }>('/payments/verify-otp', { method: 'POST', body: JSON.stringify({ reference, otp, deviceId }) }, true); return data.data; }
